...
nfc.on('tag', function(error, data) {
    if (error) {
        ...
        //console.log(' 111 userIdLittleEndian -> ' + userIdLittleEndian);
        if (userIdLittleEndian){
            // for buy product
            var prId = [0x07, 0x11];
...
function sendMsgToGloLime(address, _frameId, comandCode, cmdData){
    ...
    client.connect({host: "192.168.91.80", port: 6767},  function(socket) {
      ...
      console.log('DATA TO SEND arr:: ' + msg);
      var s = "";
      for (var i = 0; i < msg.length; i++)
      {
          s += String.fromCharCode(msg[i]);
      }
      //console.log('DATA TO SEND str:: ' + s);
      socket.write(s);
      socket.on('data', function(data){
        //console.log('RESP::');
        for(var i = 0; i < data.length; i++)
        {
          putByte(data.charCodeAt(i));
        }
      });
...
function checkCRC16_CCITT(resp){
    ...
    if (tmp1 != respCrc[0]){
      ...
      console.log(' !Attention! CRC is correct');
      return true;
    }
}
...
function putByte(cmdByte){
	switch(cmdByte)
	{
		...
		case POSTAMBLE:
			...
			fireParseComplete(); // !!!! empty function
			break;
		default:
			processByte(cmdByte);
			break;
